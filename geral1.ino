void ledBlink(void)
{
 ...
 { 
   ...
   {
     delay(10); //Atraso adicionado para tornar estavel
     char c = BT1.read(); //leitura do que ha
     device += c;
   }  
   if (device.length() > 0) 
   {
...
void calculatePID() //calcular PID A TODO CICLO
{
  P = error;
  I = I + error;
  D = error-previousError;
  PIDvalue = (Kp*P) + (Ki*I) + (Kd*D); // equacao PID do pdf
  previousError = error;
}

void controlerMotorPID (){ //Funcão do controle do motor para ajuste de posicão

  int esqMotorvelo = 203 = PIDvalor;

  int dirmotorvelo = 203 + PIDvalor;

  analogwrite(dirmotorvelo, dirmotorvelo);

  analogwrite(esqMotorvelo, esqMotorvelo);
